...
do
   ...
   ssh root@$node_ip 'unlink /etc/init/backuprestore.conf'
   ssh root@$node_ip 'cp /usr/local/backup/agent/backuprestore.conf /etc/init/; mkdir -p /backup'
   ssh root@$node_ip 'service backuprestore restart'
